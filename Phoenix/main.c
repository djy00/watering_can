...
#define LED_INTENSITY_HIGH 			(200U)  /**< High intensity. */
#define LED_INTENSITY_LOW       (50U)   /**< Low intensity. */
...
{
    NRF_TIMER1->CC[0] = new_setting;

}
static void start_init()
{

		 //nrf_gpiote_task_config(0,IN20, NRF_GPIOTE_POLARITY_TOGGLE, 
     //                      NRF_GPIOTE_INITIAL_VALUE_LOW); 
														 											 
 }		


static void gpiote_init(ble_nus_t * p_nus, uint8_t * p_data, uint16_t length)
{
    ...
    nrf_gpio_cfg_output(IN12);
		nrf_gpio_cfg_output(IN20);	 
		nrf_gpio_cfg_output(IN27); 
		nrf_gpio_cfg_output(IN24);
    ...
    /* Note that we can only connect one GPIOTE task to one output pin. */                                           
      ...
      if(strstr((char*)p_data, GO)){
         ...
         nrf_gpiote_task_config(0,IN7 ,NRF_GPIOTE_POLARITY_TOGGLE, \
				...
				nrf_gpiote_task_config(0,IN24 ,NRF_GPIOTE_POLARITY_TOGGLE, \
                          NRF_GPIOTE_INITIAL_VALUE_HIGH);
				
      }                                
                        //b
      if(strstr((char*)p_data,BACK )){
      ...
      nrf_gpiote_task_config(0,IN12, NRF_GPIOTE_POLARITY_TOGGLE, \
                           NRF_GPIOTE_INITIAL_VALUE_HIGH);
      }                           
			
                              //L
   if(strstr((char*)p_data, left)){
      nrf_gpiote_task_config(0,IN3, NRF_GPIOTE_POLARITY_TOGGLE, \
                           NRF_GPIOTE_INITIAL_VALUE_HIGH);
      nrf_gpiote_task_config(0,IN7, NRF_GPIOTE_POLARITY_TOGGLE, \
                          NRF_GPIOTE_INITIAL_VALUE_HIGH);
   ...
   if(strstr((char*)p_data, spinR)){
	...
	if(strstr((char*)p_data, shoot)){

			nrf_gpiote_task_config(0,IN27, NRF_GPIOTE_POLARITY_TOGGLE, \
                          255UL);		
	}
      
   if(strstr((char*)p_data, stop)){
		 nrf_gpiote_task_config(0,IN20, NRF_GPIOTE_POLARITY_TOGGLE, \
                           NRF_GPIOTE_INITIAL_VALUE_HIGH);}   
}
...
static void services_init(void)
{
    ...
    nus_init.data_handler = gpiote_init;
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    ble_gap_enc_info_t *             p_enc_info;
		...
		/*nrf_gpio_pin_set(IN23);
		nrf_gpio_pin_set(IN24);
		nrf_gpio_pin_clear(IN25);
		nrf_gpio_pin_clear(IN26);
		nrf_delay_ms(100);
		nrf_gpio_pin_clear(IN23);
		nrf_gpio_pin_clear(IN24);
		nrf_gpio_pin_set(IN25);
		nrf_gpio_pin_set(IN26);
		nrf_delay_ms(100);*/
